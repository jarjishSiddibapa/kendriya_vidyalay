...
public class DBManager {
	...
	public boolean isMapExist(int id1, int id2, Table_ID id1_name, Table_ID id2_name, Table_Map table_Map) {
		try {
			prepareStatement = connection.prepareStatement("select 1 from " + table_Map + " where " + id1_name
					+ "=? and " + id2_name + "=? and  is_active = TRUE");
	...
	public boolean isExist(int id, Table_ID table_id, Table table) {
		try {
			...
			prepareStatement = connection
					.prepareStatement("select 1 from " + table + " where " + table_id + "=? and is_active = TRUE");
	...
	public boolean isStringExist(String str, String column_name, Table table) {
		try {
			prepareStatement = connection
					.prepareStatement("select 1 from " + table + " where " + column_name + "=? and  is_active = TRUE");
